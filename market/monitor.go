...
func (m *WSMonitor) initializeHistoricalData() error {
	...
	for _, symbol := range m.symbols {
		...
		go func(s string) {
			...
			// 获取历史K线数据 - 5m
			klines, err := apiClient.GetKlines(s, "5m", 333)
			if err != nil {
			...
			// 获取历史K线数据 - 1h
			klines1h, err := apiClient.GetKlines(s, "1h", 333)
			if err != nil {
				log.Printf("获取 %s 历史数据失败: %v", s, err)
				return
			}
			...
			// 获取历史K线数据 - 4h
			klines4h, err := apiClient.GetKlines(s, "4h", 333)
			if err != nil {
				log.Printf("获取 %s 历史数据失败: %v", s, err)
				return
			}
			...
			// 获取历史K线数据 - 1d
			klines1d, err := apiClient.GetKlines(s, "1d", 333)
			if err != nil {
				log.Printf("获取 %s 历史数据失败: %v", s, err)
				return
			}
			if len(klines1d) > 0 {
				entry1d := &KlineCacheEntry{
					Klines:     klines1d,
					ReceivedAt: time.Now(),
				}
				m.klineDataMap1d.Store(s, entry1d)
...
func (m *WSMonitor) GetCurrentKlines(symbol string, duration string) ([]Kline, error) {
	...
	if !exists {
		...
		apiClient := NewAPIClient()
		klines, err := apiClient.GetKlines(symbol, duration, 333)
		if err != nil {
